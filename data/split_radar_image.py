...
lidar_save_path = "/data/ITRI/scene_1/processed-1600/lidar_mask_400_crop_no_morph"
...
pred_mask_path = "/data/ITRI/scene_1/processed-1600/itri-1600-50finetune"
pred_mask_paths = sorted(os.listdir(pred_mask_path))
...
for path in tqdm(radar_paths[3:]):
    ...
    #    lidar_morph = cv2.dilate(lidar_list[i], krenel, iterations=1)
    #    #cv2.imshow("lidar", lidar_list[i])
    #    #cv2.waitKey(1)
    #    cv2.imwrite(
    #        os.path.join(lidar_save_path, path[:-4] + "_" + str(i) + ".png"),
    #        lidar_morph)
    pred_mask_filename = os.path.join(pred_mask_path, path)
    pred_mask_image = cv2.imread(pred_mask_filename)
    mask_list = split_4_fold(pred_mask_image)
    for i in range(len(mask_list)):
        #cv2.imshow("lidar", lidar_list[i])
        #cv2.waitKey(1)
        cv2.imwrite(
            os.path.join(pred_mask_path, path[:-4] + "_" + str(i) + ".png"),
            mask_list[i])
