...
class BoxInteraction(Interaction):
    ...
    def end_path(self):
        ...
        self.current_mask = np.logical_or(
            self.history[-1], self.get_mask()
        )  # get the top of history and edit
    ...
    def get_mask(self):
        ...
        input_box = np.array(
            [
                int(self.all_paths[-2][1][0][0]),
                int(self.all_paths[-2][1][0][1]),
                int(self.all_paths[-2][1][-1][0]),
                int(self.all_paths[-2][1][-1][1]),
            ]
        )
